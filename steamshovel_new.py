...
def plotting():
    ...
    if 'corsika' in filelist[frame] or 'numu' in filelist[frame] or 'nugen' in filelist[frame]:
    ...
    if icetop_display: # also show IT pulses
        try:
            string = np.concatenate((string,f.root.OfflineIceTopHLCVEMPulses.cols.string[:]))
            om = np.concatenate((om,f.root.OfflineIceTopHLCVEMPulses.cols.om[:]))
            charge = np.concatenate((charge,f.root.OfflineIceTopHLCVEMPulses.cols.charge[:]))
            time = np.concatenate((time,f.root.OfflineIceTopHLCVEMPulses.cols.time[:]))
        except:
            pass
        string = np.concatenate((string,f.root.OfflineIceTopSLCVEMPulses.cols.string[:]))
        om = np.concatenate((om,f.root.OfflineIceTopSLCVEMPulses.cols.om[:]))
        charge = np.concatenate((charge,f.root.OfflineIceTopSLCVEMPulses.cols.charge[:]))
        time = np.concatenate((time,f.root.OfflineIceTopSLCVEMPulses.cols.time[:]))
    ...
    o_ = np.linspace(1,62,62,dtype=int) # both in-ice and IT
        ...
        for o in o_:
            if s not in DeepCoreStrings: # remove deepcore from data
                ...
                if total_q != 0:
                    ...
                    # print(s,o,dom_to_coord[(s,o)][0],dom_to_coord[(s,o)][1],times)
    ...
    # ------else (or after time-window cleaning): ------ # 
    data_new = data    
    ...
    def show_pulses(data_new):
        ...
        ax.text2D(-0.1,0.94, s='chi2_red_new: %.3f'%np.log10(f.root.Collection.cols.chi2_red_new[:])[0],transform=ax.transAxes,fontsize=9)
    ...
    print(data[:,3][data[:,2]>1000])
    ax2 = plt.subplot(gs[3])
    ax2.hist(data[:,4][data[:,2]<1000],bins=np.linspace(np.min(data[:,4]),np.max(data[:,4]),100),histtype='step',weights=data[:,3][data[:,2]<1000],log=True,label='in-ice',bottom=1e-1) # use OLD data array!
    ax2.hist(data[:,4][data[:,2]>1000],bins=np.linspace(np.min(data[:,4]),np.max(data[:,4]),100),histtype='step',weights=np.ones_like(data[:,4][data[:,2]>1000])+10,log=True,color='r',label='IT',bottom=1e-1) # use OLD data array!
    ...
    L = plt.axes([0.13, 0.24, 0.1, 0.03], facecolor='w')
    R = plt.axes([0.13, 0.20, 0.1, 0.03], facecolor='w')

    leftbar = Slider(L, 'left ', np.min(data[:,4]), center, valinit=lower, valstep=200,dragging=False,valfmt='')
    rightbar = Slider(R, 'right ', center, np.max(data[:,4]), valinit=upper,valstep=200,dragging=False,valfmt='')
