...
IT_om = [61, 62] # 2 tanks for each string, after merging high-gain and low-gain doms
IT_zcoord = 700
...
data_IT = []
s_ = np.linspace(1,86,86,dtype=int)
o_ = np.linspace(61,62,2,dtype=int) # including IT
for s in s_:
    for o in o_:
        if s not in DeepCoreStrings: # remove deepcore from data
            ...
            if total_q != 0:
                ...
                times = time_IT[sel]
                weighted_time = np.sum(charges*times) / np.sum(charges)
                earliest_time = np.min(time_IT[sel])
